...
import 'package:flutter_template_name/src/common/util/color_util.dart';
...
import 'package:flutter_template_name/src/common/widget/not_found_screen.dart';
import 'package:flutter_template_name/src/common/widget/outlined_text.dart';
import 'package:flutter_template_name/src/common/widget/scaffold_padding.dart';
import 'package:flutter_template_name/src/feature/shop/model/product.dart';
import 'package:flutter_template_name/src/feature/shop/widget/catalog_breadcrumbs.dart';
import 'package:flutter_template_name/src/feature/shop/widget/favorite_button.dart';
import 'package:flutter_template_name/src/feature/shop/widget/shop_back_button.dart';
...
class _ProductStarsState extends State<_ProductStars> with SingleTickerProviderStateMixin {
          ...
          children: <Widget>[
            ...
            ..._icons.mapIndexed(
              (i, icon) => AnimatedBuilder(
                ...
                builder: (context, _) {
                  var angle = 2 * math.pi / 5 * i + (2 * math.pi * _controller.value);
  ...
  Widget build(BuildContext context) => SliverPadding(
        ...
        sliver: SliverToBoxAdapter(
          child: Wrap(
            ...
            children: <Widget>[
              for (var i = 0; i < colors.length; i++) _ProductTag('Tag', 'Value ${i + 1}', color: colors[i]),
            ],
          ),
        ),
      );
        ...
        sliver: SliverFixedExtentList.list(
          ...
          children: <Widget>[
            ...
            if (product.discountPercentage >= 1)
              ProductProperty(title: 'Discount', value: '${product.discountPercentage.round()}%'),
...
class _ProductPhotosListViewState extends State<_ProductPhotosListView> {
  ...
  ImageProvider<Object> _getImageProvider(String path) =>
      (!kIsWeb || Config.environment.isDevelopment ? AssetImage(path) : NetworkImage('/$path'))
  ...
  Widget build(BuildContext context) => ShaderMask(
        shaderCallback: (rect) => const LinearGradient(
          ...
          colors: <Color>[Colors.purple, Colors.transparent, Colors.transparent, Colors.purple],
          stops: <double>[0, 0.15, 0.85, 1], // 15% purple, 70% transparent, 15% purple
        ).createShader(rect),
...
class _ProductPropertyDotsPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _ProductPropertyDotsPainter oldDelegate) => false;

  @override
  bool shouldRebuildSemantics(covariant _ProductPropertyDotsPainter oldDelegate) => false;
}
